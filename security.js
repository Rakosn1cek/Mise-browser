...
function hardenSession(targetSession) {
    ...
    // Sec-CH-UA / Sec-CH-UA-Full-Version-List / Sec-CH-UA-Platform etc. are
    // generated by Chromium internally from the real brand/version list,
    // independent of session.setUserAgent(), and can re-expose the real
    // Chrome/Electron build even after the User-Agent header is spoofed.
    //
    // Deliberately NOT touching Accept or Accept-Language here (tried it,
    // made things worse): a hand-typed "realistic Chrome" Accept header
    // came back rarer among real Cover Your Tracks visitors (1-in-6402)
    // than Electron's own untouched default (1-in-247). Blending in means
    // matching what the crowd actually sends, not what looks plausible —
    // Electron's real default is closer to that than a guess is.
